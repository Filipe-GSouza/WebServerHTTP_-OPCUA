...
@retry(stop_max_attempt_number=3, wait_fixed=2000)  # Tenta no máximo 3 vezes, esperando 2 segundos entre cada tentativa
def fazer_requisicao(url, username, password):
   ...
   #---------------------------------------------------------------------------------------------------- 
    ...
    try:
        while True:
            ...
            while True:
                try:
                    # Tenta fazer as requisições
                    response1 = fazer_requisicao(url_regard_1, username, password)
                    response2 = fazer_requisicao(url_regard_2, username, password)
                    print("Requisições bem sucedidas.")
                    # Se as requisições foram bem sucedidas, saia do loop
                    time.sleep(1)
                    
                except Exception as e:
                    print("Erro ao fazer requisição:", e)
                    print("Tentando novamente...")
                    time.sleep(1)  # Espera 1 segundo ante de tentar novamente
